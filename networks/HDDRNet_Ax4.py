    ...
    def build_model(self, x, is_training, reuse, verbose):
        with tf.variable_scope('lfresnet', reuse=reuse):
            ...
            with tf.variable_scope('conv4d3'):
                x = conv4d(x, 64, 10 * 8 * 8, kernel_size_1=3, kernel_size_2=5, padding='SAME', trainable=True)
                x = angular_pixel_shuffle(x, rv_in=2, rv_out=8)
                x = leakyrelu(x)

            # ====================== Spatial Details Recovery ====================== #
            with tf.variable_scope('conv4d4'):
                x = conv4d(x, 40, 64, kernel_size_1=3, kernel_size_2=3, padding='SAME', trainable=True)
                source2 = x
