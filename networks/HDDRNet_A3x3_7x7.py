    ...
    def build_model(self, x, is_training, reuse, verbose):
        with tf.variable_scope('lfresnet', reuse=reuse):
            ...
            with tf.variable_scope('conv4d3'):
                x = conv4d(x, 64, 6*7*7, kernel_size_1=3, kernel_size_2=5, padding='SAME', trainable=True)
                x = angular_pixel_shuffle(x, rv_in=3, rv_out=7)
                x = leakyrelu(x)

            # ====================== Spatial Details Recovery ====================== #
            with tf.variable_scope('conv4d4'):
                x = conv4d(x, 54, 64, kernel_size_1=3, kernel_size_2=3, padding='SAME', trainable=True)
                source2 = x
