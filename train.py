...
parser.add_argument("--datadir", type=str, default="./", help="The training and testing data path")
...
parser.add_argument("--perceptual_loss", type=bool, default=True, help="Need to use perceptual loss or not, if true, need to set the vgg_model item")
...
def import_model(scale_S, scale_A):
    if scale_A == 1:
        ...
        if scale_A == 2:
            from networks.HDDRNet_A5x5_9x9 import HDDRNet
        if scale_A == 3:
            from networks.HDDRNet_A3x3_9x9 import HDDRNet
        if scale_A == 4:
            from networks.HDDRNet_A2x2_8x8 import HDDRNet
    else:
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.num_epoch):
        ...
        for ii in range(num_iter):
            ...
            logging.info("Epoch {:03d} [{:03d}/{:03d}]   Angular loss: {:.6f} | Spatial loss: {:.6f} | "
